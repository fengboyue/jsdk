...
(function () {
    ...
    let _remove = function(this:HTMLElement|Window, type, fn?:Function){
        if(!fn) return;
    ...
    _removes = function(this:HTMLElement|Window, type, fns:Function[]){
        if(fns) fns.forEach(f=>{_remove.call(this, type, f)})
    },
...
module JS {
    ...
    export namespace util {
        ...
        let _head = ()=>{return document.querySelector('head')},
        ...
        export class Dom {
            ...
            public static rename(node: Element, newTagName: string) {
                let newNode = document.createElement(newTagName), aNames:string[] = node['getAttributeNames']();
            ...
            public static applyHtml(html: string | Document, appendTo?: string | HTMLElement, ignore?: { script?: boolean, css?: boolean }|boolean): Promise<string> {
                ...
                return Promises.create<string>(function () {
                    let doc = typeof html == 'string' ? new DOMParser().parseFromString(html, 'text/html') : <Document>html,
                        url = doc.URL,
                        el = Dom.$1(appendTo || document.body);
