        ...
        export class Tab extends Widget {
            ...
            private _html() {
                ...
                let hHtml = `<ul id="${this.id}_headers" role="tablist" class="nav${cls} ${cfg.headCls || ''}" style="${cfg.headStyle || ''}">${heads}</ul>`,
                    cHtml = `<div class="${isVtl ? 'vertical' : ''} tab-content" style="height:${Lengths.toCssString(cfg.height, '100%')};">${contents}</div>`,
                    leftWidth = Lengths.toCssString(cfg.headLeftWidth, '100%');
